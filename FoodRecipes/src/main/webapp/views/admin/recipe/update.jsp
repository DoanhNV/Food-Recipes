		...
		<div class="child-pop ">
			<c:forEach items="${list_checked_material}" var="material">
				<div class="child-content ">
					<div>
						<input type="checkbox" value="${material.id}" ${material.checked} class="check-mater" name="materialIds"> 
	...
	<div class="create create-left">
		<div class="create-left-content">
				...
				<div class="preview-recipe-profile" id="image-temp">
					<img src="${recipe.featureImage}" id="img-temp" style="width: 100%; height: 100%;"
				...
				<select class="form-control select" name="hour">
					<c:forEach items="${recipe_hours}"  var="recipeHourItem">
								<option ${recipeHourItem.selected}  value="${recipeHourItem.timeValue}">${recipeHourItem.timeName}</option>
					</c:forEach>
				</select> 
				<select class="form-control select" name="minute">
					<c:forEach items="${recipe_minutes }"  var="recipeMinuteItem">
						<option ${recipeMinuteItem.selected} value="${recipeMinuteItem.timeValue}">${recipeMinuteItem.timeName}</option>
					</c:forEach>
				</select>
				<div class="clear"></div>
				...
				<div class="input-group">
					<span class="input-group-addon"><i
						class="glyphicon glyphicon-pencil"></i></span> 
						<input type="text" name="video" id="videoField" value="${recipe.videoUrl}"
				...
				<div class="cate-step-content" id="tagRecipeCate">
					<c:forEach items="${list_checked_material}" var="material">
						<c:if test="${material.checked == 'checked' }">
							<button type="button" id="tagElement" class="btn btn-success" name="" value="1">${material.materialName}<span class="glyphicon glyphicon-tags title-material-tag"></span>
						</c:if>
					</c:forEach>
				</div>
				<div class="clear"></div>
		...
		<c:forEach items="${list_step}" var="step" varStatus="status">
			<div class="create-left-content step-practise" id="step${status.index + 1}">
				<div class="step-post step-element-number">
					<div class="number-title">${status.index + 1}</div>
				</div>
				<div class="step-post step-element-content">
					<div class="form-group">
						<textarea class="form-control baby-step-content" rows="5" name="stepText" required="required">${step.text}</textarea>
					</div>
					<div class="clear"></div>
					<input type="file" id="upload-file-step${status.index + 1}" name="stepImg"
						class="file-upload${status.index + 1}" style="display: none;" multiple />
					<button type="button" class="btn upload-step" id="uploadStep${status.index + 1}">
						<span class="glyphicon glyphicon-camera"></span>
					</button>
	
					<!-- hidden -->
					<div class="preview-step-img " id="preview${status.index + 1}" style="display:block !important;">
						<div class="preview-step-img-element">
							<img src="${step.filePath}" class="preview-element-img" id="img-preview${status.index + 1}"  />
						</div>
					</div>
				</div>
			</div>
			...
			<div class="cate-step-content">
				<c:forEach items="${list.cates}" var="cate" varStatus="status">
					<div class="checkbox cate-check ">
						<label><input type="checkbox" name="recipeCate" ${cate.checked} value="${cate.id}-${list.kindId}">${cate.title}</label>
					</div>
				</c:forEach>
			</div>
