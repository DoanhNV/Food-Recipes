...
<script
	src="<%=request.getContextPath()%>/resources/asset/js/jquery-3.2.1.min.js"></script>	
...
<body>
	<div class="login">
		<h1>Login</h1>
		<form method="post" modelAttribute="adminUser" id="loginForm">
			<h4 class="error-message">
				<c:out value="${admin_login_fail}" />
			</h4>
			<input type="text" name="userName" placeholder="Username"
				id="txtUserName" />
			<h4 class="error-message" id="txtUserNameErr"></h4>
			<input type="password" name="password" placeholder="Password"
				id="txtPassword" />
			<h4 class="error-message" id="txtPasswordErr"></h4>
		...
		<script type="text/javascript">
			...
			function login() {
				...
				if ($("#txtPassword").val() == "") {
					isValid = false;
					$("#txtPasswordErr").text("Bạn phải nhập tên đăng nhập");
				} else {
					$("#txtPasswordErr").text("");
				}
				if (isValid) {
					$("#loginForm").attr("action", "../admin/doLogin");
					$("#loginForm").submit();
				}
			}
		</script>
